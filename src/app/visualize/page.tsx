...
const EmbeddingVisualizer: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setAdditionalInfo(`Error generating embeddings: ${(error as Error).message}`);
    }
  };
  ...
  function getColor(value: number, minValue: number, maxValue: number): string {
    ...
    const transformedValue = Math.sign(normalizedValue - 0.5) * Math.pow(Math.abs(normalizedValue - 0.5) * 2, 0.65) / 2 + 0.5;
  ...
  return (
    <div className="w-full p-10 pt-20 space-y-8">
      <div className="grid grid-cols-3 gap-4">
        <div className="space-y-2">
          <label htmlFor="input1" className="block">Input 1</label>
          <input
            id="input1"
        ...
        <div className="space-y-2">
          ...
          <div className="p-2 border rounded min-h-[40px]">{additionalInfo}</div>
        </div>
        <div className="space-y-2">
          <label htmlFor="input2" className="block">Input 2</label>
          <input
            id="input2"
            className="w-full p-2 border rounded text-black"
            value={input2}
      ...
      <div className="grid grid-cols-2 gap-16">
        <div className="flex flex-col items-center">
          <h3 className="text-lg font-bold mb-4 text-center">Heatmap for Input 1</h3>
          {embedding1.length > 0 && <HeatmapComponent embedding={embedding1} />}
        </div>
        <div className="flex flex-col items-center">
          <h3 className="text-lg font-bold mb-4 text-center">Heatmap for Input 2</h3>
